...
public class Decorator extends DefaultFieldDecorator {
    ...
    public Object decorate(ClassLoader loader, Field field) {
        ...
        }else if (field.getType().equals(DropDownMenu.class)){
            ElementLocator locator = factory.createLocator(field);
            if (locator == null){
                return null;
            }
            return new DropDownMenu(proxyForLocator(loader, locator));
        }else if (field.getType().equals(Product.class)){
            ElementLocator locator = factory.createLocator(field);
            if (locator == null){
                return null;
            }
            return new Product(proxyForLocator(loader, locator));
        }else if (field.getType().equals(FieldWithData.class)){
            ElementLocator locator = factory.createLocator(field);
            if (locator == null){
                return null;
            }
            return new FieldWithData(proxyForLocator(loader, locator));
        }
